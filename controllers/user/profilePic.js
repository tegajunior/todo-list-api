...
const dpUpload = async (req, res) => {
  ...
  if (!loggedInUser)
    return res
      .status(404)
      .json({ msg: "Please login to continue", success: false });

  await cloudinarySetup();

  const uploadedImage = await cloudinary.uploader.upload(req.file.path, {
    eager: [
      { height: 55, width: 55, crop: "fill" },
  ...
  if (!uploadedImage)
    return res
      .status(500)
      .json({ msg: "An error occurred while uploading", success: false });
  ...
  return res
    .status(201)
    .json({ msg: "Profile image updated successfully", success: true, user: loggedInUser });
};
